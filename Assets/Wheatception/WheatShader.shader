	...
	SubShader
	{
		...
		Pass
		{
			...
			void geo(point v2g p[1], inout TriangleStream<g2f> triStream)
			{
				...
				o.viewDir = WorldSpaceViewDir(topPointA + p[0].BasePoint);
				o.uvs = float2(0, 1);
				triStream.Append(o);
				
				o.vertex = UnityObjectToClipPos(topPointB + p[0].BasePoint);
				o.viewDir = WorldSpaceViewDir(topPointB + p[0].BasePoint);
				o.uvs = float2(1, 1);
				triStream.Append(o);
				
				o.vertex = UnityObjectToClipPos(bottomPointA + p[0].BasePoint);
				o.viewDir = WorldSpaceViewDir(bottomPointA + p[0].BasePoint);
				o.uvs = float2(0, 0);
				triStream.Append(o);
				
				o.vertex = UnityObjectToClipPos(bottomPointB + p[0].BasePoint);
				o.viewDir = WorldSpaceViewDir(bottomPointB + p[0].BasePoint);
				o.uvs = float2(1, 0);
				triStream.Append(o);
			}
			...
			fixed4 frag (g2f i) : SV_Target
			{
				...
				float3 skybox = texCUBE(_Skybox, -i.viewDir);

				float3 highColor = _HighColor + (_HighColor * i.planeDistToCenter);
				float3 color = lerp(_LowColor, highColor, pow(i.uvs.y, 2));
				color = lerp(color, skybox, saturate(i.planeDistToCenter));
				return float4(color + effectorLight / 2, 1);
			}
			ENDCG
		}
	}
